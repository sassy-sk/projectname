...
## 使用環境
- Node.js バージョン（現状検証してないので不明、とりあえず23.11.0で動作してます）
- npm バージョン（現状検証してないので不明、とりあえず10.9.2で動作してます）

## 使い方および操作方法  
- ビルドした際はpublicのjpg、pngをassetsにwebp変換出力する仕様となります。  
- src内のhtml、ejs、scssでは元画像のjpg,pngで記載してください。ビルド時にwebpに変換されます。 
- ejs仕様：ページファイルはsrc内のhtml拡張子を使用、パーツファイルはejs拡張子を使用してます（vite-ejsプラグインの使用上、ページ用のファィルはhtml拡張子ではないと認識しないため）
